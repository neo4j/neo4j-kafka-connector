...
private class DatabasesWaitStrategy(private val auth: AuthToken) : AbstractWaitStrategy() {
  ...
  override fun waitUntilReady() {
    val boltUrl =
      "bolt://${waitStrategyTarget.containerIpAddress}:${waitStrategyTarget.getMappedPort(7687)}"
          ...
          val databasesStatus =
            systemSession.beginTransaction().use { tx ->
              tx
                .run("SHOW DATABASES")
                .list()
                .map { it.get("name").asString() to it.get("currentStatus").asString() }
                .toMap()
            }
          val notOnline = databasesStatus.filterValues { it != "online" }
          if (databasesStatus.size < databases.size || notOnline.isNotEmpty()) {
            throw RuntimeException(
              "Cannot started because of the following databases: ${notOnline.keys}")
          }
        }
      }
      true
...
class Neo4jContainerExtension(dockerImage: String) :
  Neo4jContainer<Neo4jContainerExtension>(dockerImage) {
  ...
  private fun createAuth(): AuthToken {
    return if (!adminPassword.isNullOrBlank()) AuthTokens.basic("neo4j", adminPassword)
    else AuthTokens.none()
  }
  ...
  override fun start() {
    ...
    if (waitStrategies.isNotEmpty()) {
      ...
      waitStrategies.reversed().forEach { waitStrategy ->
  ...
  private fun createDriver() {
    ...
    cypher?.split(";")?.forEach { query -> session!!.beginTransaction().use { it.run(query) } }
  }
  ...
  private fun findDistrFile(): File? {
    try {
      return File("../target/containerPlugins")
        .listFiles()
