...
class Neo4jNewSinkTask : SinkTask() {
  private val log: Logger = LoggerFactory.getLogger(Neo4jNewSinkTask::class.java)

  private lateinit var settings: Map<String, String>
  private lateinit var config: SinkConfiguration
  private lateinit var topicHandlers: Map<String, SinkStrategyHandler>

  override fun version(): String = VersionUtil.version(Neo4jNewSinkTask::class.java)
          ...
          txGroups.forEach { group ->
            config.session().use { session ->
              session.writeTransaction(
                  { tx -> group.forEach { tx.run(it).consume() } }, config.txConfig())
            }
          }
        }
  }
