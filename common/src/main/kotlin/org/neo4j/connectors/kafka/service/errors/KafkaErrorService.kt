...
import org.neo4j.connectors.kafka.utils.ValidationUtils.validateConnection

class KafkaErrorService(
  ...
  constructor(
      ...
      log: (String, Exception?) -> Unit
  ) : this(producer(errorConfig, config, log), errorConfig, log)
        ...
        errorConfig.dlqTopic?.let {
          try {
            val bootstrapServers =
                config.getOrDefault(CommonClientConfigs.BOOTSTRAP_SERVERS_CONFIG, "").toString()
            validateConnection(
                bootstrapServers, CommonClientConfigs.BOOTSTRAP_SERVERS_CONFIG, false)
  ...
  override fun report(errorDatas: List<ErrorData>) {
    ...
    if (errorConfig.log) {
      if (errorConfig.logMessages) {
        ...
        errorDatas
            .map { it.exception }
            .distinct()
            .forEach { log("Error processing ${errorDatas.size} messages", it) }
      }
    }
    ...
    errorDatas.forEach { dlqData ->
      producer?.let {
        try {
          val producerRecord =
              if (dlqData.timestamp == RecordBatch.NO_TIMESTAMP) {
                ProducerRecord(errorConfig.dlqTopic, null, dlqData.key, dlqData.value)
              } else {
                ProducerRecord(
                    errorConfig.dlqTopic, null, dlqData.timestamp, dlqData.key, dlqData.value)
              }
          if (errorConfig.dlqHeaders) {
            val producerHeader = producerRecord.headers()
            populateContextHeaders(dlqData).forEach { (key, value) ->
  ...
  fun populateContextHeaders(errorData: ErrorData): Map<String, ByteArray> {
    ...
    if (errorData.exception != null) {
      ...
      headers[prefix("exception.stacktrace")] =
          ExceptionUtils.getStackTrace(errorData.exception).toByteArray()
    }
    return headers
  }
