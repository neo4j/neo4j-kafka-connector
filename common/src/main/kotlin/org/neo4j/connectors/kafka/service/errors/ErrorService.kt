...
data class ErrorData(
  ...
  companion object {
    ...
    fun from(
        consumerRecord: ConsumerRecord<out Any, out Any>,
        exception: Exception?,
        executingClass: Class<*>?,
        databaseName: String?
    ): ErrorData {
      return ErrorData(
          offset = consumerRecord.offset().toString(),
...
abstract class ErrorService(private val config: Map<String, Any> = emptyMap()) {
  ...
  data class ErrorConfig(
      ...
      val dlqHeaderPrefix: String = "",
      val dlqHeaders: Boolean = false,
      val dlqReplication: Int? = 3
  ) {
...
class ProcessingError(val errorDatas: List<ErrorData>) :
    RuntimeException(
        "Error processing ${errorDatas.size} messages\n" +
