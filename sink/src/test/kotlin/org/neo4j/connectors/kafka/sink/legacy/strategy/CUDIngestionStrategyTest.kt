...
class CUDIngestionStrategyTest {
  ...
  private fun assertRelationshipEventsContainsKey(qe: QueryEvents, fromKey: String, toKey: String) =
      assertTrue {
        qe.events.all {
  ...
  fun `should create, merge, update and delete nodes with garbage data`() {
    ...
    val list =
        (1..10).map {
          val labels =
              if (it % 2 == 0) listOf("WellBehaved", "C̸r̵a̵z̵y̵ ̶.̵ ̶ ̴ ̸ ̶ ̶ ̵ ̴L̴a̵b̸e̶l")
              else listOf("WellBehaved", "C̸r̵a̵z̵y̵ ̶.̵ ̶ ̴ ̸ ̶ ̶ ̵ ̴L̴a̵b̸e̶l", "Label")
          val properties =
              if (it in deleteMarkers) emptyMap() else mapOf("foo" to "foo-value-$it", "id" to it)
          val (op, ids) =
              when (it) {
                in mergeMarkers -> CUDOperations.merge to mapOf(key to it)
                in updateMarkers -> CUDOperations.update to mapOf(key to it)
        ...
        (1..10).map {
          ...
          val start =
              CUDNodeRel(
                  ids = mapOf(key to it),
                  labels = labels,
                  op = if (it in invalidMarker) CUDOperations.delete else CUDOperations.create)
          val end = CUDNodeRel(ids = mapOf(key to it + 1), labels = labels)
          val rel =
              CUDRelationship(
                  op = op, properties = properties, from = start, to = end, rel_type = "MY_REL")
  ...
  fun `should delete nodes with internal id reference`() {
    ...
    val deleteNodeFooBarLabelDetach =
        findEventByQuery(
            """
                |${StreamsUtils.UNWIND}
                |MATCH (n) WHERE id(n) = event.${CUDIngestionStrategy.ID_KEY}._id
  ...
  fun `should create and delete relationship also without properties field`() {
    ...
    val list =
        listOf(CUDOperations.create, CUDOperations.delete, CUDOperations.update).map {
  ...
  fun `should create, merge and update relationships with merge op in 'from' and 'to' node`() {
    ...
    val list =
        (1..10).map {
          ...
          val end =
              CUDNodeRel(ids = mapOf(key to it + 1), labels = labels, op = CUDOperations.merge)
          val rel =
              CUDRelationship(
                  op = op, properties = properties, from = start, to = end, rel_type = "MY_REL")
          StreamsSinkEntity(null, rel)
  ...
  fun `should create, merge and update relationships with match op in 'from' node and merge or create in 'to' node`() {
    ...
    val list =
        (1..10).map {
          ...
          val end =
              CUDNodeRel(
                  ids = mapOf(key to it + 1),
                  labels = labels,
                  op = if (it <= 5) CUDOperations.merge else CUDOperations.create)
          val rel =
              CUDRelationship(
                  op = op, properties = properties, from = start, to = end, rel_type = "MY_REL")
          StreamsSinkEntity(null, rel)
    ...
    assertRelationshipEventsContainsKey(mergeRelFooBar, key, key)
    val matchMergeAndMergeRelFooBar =
        findEventByQuery(
            """
                |${StreamsUtils.UNWIND}
                |MATCH (from:Foo:Bar {key: event.from.${CUDIngestionStrategy.ID_KEY}.key})
    ...
    val matchMergeAndCreateRelFooBar =
        findEventByQuery(
            """
                |${StreamsUtils.UNWIND}
                |MATCH (from:Foo:Bar {key: event.from.${CUDIngestionStrategy.ID_KEY}.key})
